...
document.onreadystatechange = () => {
	if (document.readyState === 'interactive') {
		...
		document.querySelectorAll('#rwtable thead th').forEach((x) =>
...
function updateFilters() {
	...
	let checked_item_types = Array.from(
		document.querySelectorAll('#bases input[type=checkbox]:checked')
	).map((x) => x.value);
	let versions = Array.from(
		document.querySelectorAll('#filter_versions input[type=checkbox]:checked')
	).map((x) => x.value);
	let required_runes = Array.from(
		document.querySelectorAll('#required_runes input[type=checkbox]:checked')
	).map((x) => x.value);

	let [minsocket, maxsocket] = [
		document.getElementById('minsocket'),
		document.getElementById('maxsocket')
	];
	...
	let [minlevel, maxlevel] = [
		document.getElementById('minlevel'),
		document.getElementById('maxlevel')
	];
	...
	for (let i = 0; i < RUNEWORDS_DATA.length; i++) {
		...
		let show = true;
		if (
			(!ladder_d2lod && rw.ladder.d2lod) ||
			(!ladder_d2r && rw.ladder.d2r) ||
			(only_can_make && !rw.success) ||
			versions.indexOf(rw.version) == -1 ||
			rw.sockets < minsocket.value ||
			rw.sockets > maxsocket.value ||
			rw.levelreq < minlevel.value ||
			rw.levelreq > maxlevel.value ||
	...
	if (mark_instance != null) {
		mark_instance.unmark();
	}
	mark_instance = new Mark(document.querySelectorAll('#rwtable tr:not([hidden]) .searchable')).mark(
		search_term,
		{ separateWordSearch: false }
	);

	document.getElementById('runeword-nb').innerText =
		`Showing: ${shown_nb}/${RUNEWORDS_DATA.length} runewords (${((shown_nb / RUNEWORDS_DATA.length) * 100).toFixed(2)}%)`;
}
...
function makeTableEntryBases(runeword_data) {
	function makeBasesHTML(bases) {
		return bases
			.map((x) =>
				x in ITEM_TYPES
					? `<em data-html="true" data-tooltip="${ITEM_TYPES[x].join(',\n')}">${x}</em>`
					: `<span>${x}</span>`
			)
			.join('<br />');
	}
	...
	if (runeword_data.bases_d2r.length == 0) {
		...
		d2lod_warning.setAttribute(
			'data-tooltip',
			'Bases available in Diablo II: Lord of Destruction.'
		);
...
function makeTableEntryStats(runeword_data) {
	...
	stats.innerHTML = runeword_data.stats
		.map((x) =>
			x.toLowerCase().includes('varies') ? "<span class='varying-stat'>" + x + '</span>' : x
		)
		.join('<br />');
	return stats;
}
...
function makeTableEntryCubeReq(runeword_data) {
	...
	cubing_required.innerHTML = runeword_data.success
		? formatUpgs(runeword_data.upgsDone, runeword_data.runes)
		: '';
